...
import { FileText, Clock, CheckCircle, XCircle, Gavel } from 'lucide-react';
  ...
  return (
    <div data-testid="customer-dashboard" className="min-h-screen bg-muted pt-20">
      <div className="max-w-7xl mx-auto px-6 md:px-12 py-12">
        <div className="mb-8">
          <h1 className="font-serif text-4xl md:text-5xl font-bold text-foreground mb-2">Customer Dashboard</h1>
        ...
        <div className="bg-white rounded-xl shadow-sm overflow-hidden">
          <div className="border-b border-border">
            <div className="flex">
              {tabs.map(t => (
                <button key={t.key} data-testid={`customer-tab-${t.key}`} onClick={() => setActiveTab(t.key)}
            ...
            {activeTab === 'quotes' && (
              quotes.length === 0 ? (
                ...
                <div className="space-y-4">
                  {quotes.map(q => (
                    <div key={q.id} data-testid={`customer-quote-${q.id}`} className="border border-border rounded-lg p-5 hover:border-primary transition-colors">
            ...
            {activeTab === 'bids' && (
              bids.length === 0 ? (
                ...
                <div className="overflow-x-auto">
                  <table data-testid="customer-bids-table" className="w-full text-sm">
                    <thead>
                      <tr className="border-b border-border">
                        ...
                        <th className="text-left py-3 px-3 font-semibold text-foreground">Currency</th>
                    ...
                    <tbody>
                      {bids.map(b => {
                        ...
                        return (
                          <tr key={b.id} data-testid={`customer-bid-${b.id}`} className="border-b border-border last:border-0 hover:bg-muted/50">
                            <td className="py-3 px-3 font-medium text-foreground">{b.product_name}</td>
                            <td className="py-3 px-3 text-muted-foreground">{b.product_size}</td>
                            <td className="py-3 px-3 text-foreground">{qty}</td>
                            <td className="py-3 px-3 text-foreground font-medium">{price}</td>
                            <td className="py-3 px-3 text-foreground">{b.currency}</td>
